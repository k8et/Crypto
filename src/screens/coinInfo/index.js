  ...
  useEffect(() => {
    ...
    const fetchHistoricalPriceData = async () => {
      try {
        ...
        setHistoricalPriceData(priceData);
      } catch (error) {
    ...
    <div className={styles.container}>
      {coinDetails ? (
        <div className={styles.coinDetails}>
          ...
          <p>
            {t("coinDetails.volume24h")}: $
            {coinDetails.market_data.total_volume.usd.toLocaleString()}
          </p>
          <p>
            {t("coinDetails.high24h")}: ${coinDetails.market_data.high_24h.usd}
          </p>
          <p>
            {t("coinDetails.low24h")}: ${coinDetails.market_data.low_24h.usd}
          </p>
          <p>
            {t("coinDetails.circulatingSupply")}:{" "}
            {coinDetails.market_data.circulating_supply.toLocaleString()}{" "}
            {coinDetails.symbol.toUpperCase()}
          </p>
          <p>
            {t("coinDetails.totalSupply")}:{" "}
